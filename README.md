...
На данной странице происходит сброс пароля пользователя. По полученным значениям электронной почты и имени пользователя ищется соответствующий пользователь. Если такой пользователь найден, то на его почту высылается временная ссылка на страницу сброса пароля - ***http://localhost/reset_password.php?token=%reset_token***, если же пользователь не найден, то под полями ввода появляется блок с сообщением о соответствующей ошибке. 
